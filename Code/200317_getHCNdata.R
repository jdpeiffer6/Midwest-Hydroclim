setwd("/home/jdp6n8/Data/Code/HCN")
library(rnoaa)
library(hydroTSM)

makeName = function(row, dataList) {
  name = paste((dataList[row, 1]), (dataList[row, 6]), (dataList[row, 9]), ".Rdata", sep =
                 "_")
  return(name)
}
...
load("uptonow.Rdata")
uptonow = subset(uptonow, element == "PRCP")
